...
const seedRangesToPlant: Array<number[]> = []
...
for (const [_lineNumber, _line] of Object.entries(lines)) {
  ...
  if (line.startsWith("seeds:")) {
    ...
    for (const seed of seeds) {
      ...
      if (iterator === 2) {
        seedRangesToPlant.push(accumulator)
        iterator = 0
        accumulator = []
      }
    }
...
for (const [seedStart, seedEnd] of seedRangesToPlant) {
  for (let i = seedStart; i < seedStart + seedEnd; i++) {
    const soil = convertSource(i, convertMaps.seedToSoil)
    ...
    const humidity = convertSource(
      temperature,
      convertMaps.temperatureToHumidity
    )
